...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        if self.display_id > 0:  # connect to a visdom server given <display_port> and <display_server>
            ...
            """    
            #自訂新增:使用ngrok
            self.visdom_proc = subprocess.Popen(["python", "-m", "visdom.server", "-p", str(self.port)])
            time.sleep(5) #開啟visdom

            # 使用ngrok公開visdom
            public_url = ngrok.connect(port=self.port)
            print("Visdom URL:", public_url)

            # 連接visdom
            self.vis = Visdom(server=public_url, env=opt.display_env)
